...
import {
	...
	SetPasswordSchema,
} from "@/app/lib/zod/schema/password-schema";
...
export async function PATCH(request: Request) {
	...
	const { oldPassword, newPassword } = await PasswordChangeSchema.parseAsync(
		await request.json(),
	);
	...
	try {
		...
		if (!password?.hashedpassword) {
			return NextResponse.json(
				{ message: "No password found. Please set your password" },
				{ status: 404 },
			);
		}
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		if (existingPassword?.hashedpassword) {
			return NextResponse.json(
				{ message: "Password already set. Please update your password" },
				{ status: 400 },
			);
		}
